...
class ccm:
	...
	def analysis(self, L):
		"""
		computes reconstructed y from x (y_Mx) and
        reconstructed x from y (x_My)
		"""
...
def prediction_general(X, tau, E, interpolate=False, plotting=False):
    """  
    E: embedding dimension
    """
...
class FNN:
    """
    Using false nearest neighbors to find optimal E with a given tau
    """
    ...
    def __init__(self, E, X, Mx, tau):
        ...
        self.tau = tau # delay
        self.X = X # one dimensional time series data
        self.Mx = Mx # shadow manifold constructed in E+1 dimension


    def distance(self, x, y):
        # L2 norm to compute distance between time series x and y
        dist = LA.norm(x-y)

        return dist

    def find_fnn(self, rho_a, Ra):

        """
        find percentage of false nearest neighbors
        """


        # find E+2 nearest neights of x_t in Mx in dimension E+1
        num_nn = self.E + 2
        N = self.Mx.shape[0]
        Ra = LA.norm(self.X - np.mean(self.X))/np.sqrt(N)
        # Ra = np.sqrt(1/N*np.sum((self.Mx[:,0] - np.mean(self.Mx[:,0]))**2))
        ...
        # Identify false nearest neighbors
        num_fnn = 0
        for i in range(self.Mx.shape[0]):
            y = self.Mx[i, :] # target point in E+1 dimension
            # extract y's nearest neighbors indices in E+1 dimension
            ynn = self.Mx[indices[i,:],:]

            # compoute the ratio in (4)
            for j in range(num_nn):
                ratio = LA.norm(y - ynn[j,:])/Ra
                if ratio > rho_a:
                    num_fnn += 1

        total_nn = num_nn*N
        perc_fnn = num_fnn/total_nn*100
